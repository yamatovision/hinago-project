...
import { ScenarioModel as MongoScenarioModel } from '../../../src/db/models/schemas/scenario.schema';
...
describe('シナリオ作成テスト - DB直接版', () => {
  ...
  it('MongoDBに直接シナリオを作成できる', async () => {
    ...
    // 検証
    expect(scenarioObject).toBeDefined();
    expect(scenarioObject.propertyId).toBe(testPropertyId);
    expect(scenarioObject.volumeCheckId).toBe(testVolumeCheckId);
    expect(scenarioObject.params.assetType).toBe(AssetType.MANSION);
